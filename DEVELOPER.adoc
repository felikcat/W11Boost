...
. Disabling VBS on UEFI Lock enabled PCs can boot-loop desktop PCs, but work as intended on laptops. Therefore, `$reduce_mitigations = 1` will only work for the majority of PCs that have CG and VBS' UEFI lock disabled.

. Per user configuration. W11Boost is meant for your machines only, not on PCs owned by your company if you lack their explicit permission to use W11Boost.
