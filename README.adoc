...
- Primarily it's to make Windows feel better than it was prior, and reduce the amount of privacy violations occuring or advertisements from showing up.
. Has a fleshed out optional anti-forensics feature. This is meant for people running programs they don't trust with their privacy, such as anti-cheats.
...
.Building
. Install MSYS2 and Git via running `winget install -eh MSYS2.MSYS2; winget install -eh Git.Git` in PowerShell.

. Open MSYS UCRT64, then run `pacman -S mingw-w64-x86_64-gcc`; this will include the Resource (.rc file) compiler that W11Boost needs to use Rust's GCC toolchain.

. Have a PowerShell open that is inside of W11Boost's main directory.

. Install Rust's nightly GCC compiler: `rustup +nightly target add x86_64-pc-windows-gnu`

. In Git Bash, run `chmod +x build_release.sh` then `./build_release.sh` to compile W11Boost in Release mode.
- You can run tests using `./tests_run.sh`, note however that this script requires Git Bash to be ran with Administrator privileges.
