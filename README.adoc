...
. Only the latest release/stable Windows 11 builds are tested.
. *It's assumed Windows is installed onto an SSD.*
- There will be unintentional slowdown on HDDs (slow disks).
. *Windows Updates will not automatically be cleared out.*
- 1. Run the built-in "Disk Cleanup" as administrator to clear out updater packages.
- 2. Read https://learn.microsoft.com/en-us/windows-hardware/manufacture/desktop/clean-up-the-winsxs-folder?view=windows-11#use-the-startcomponentcleanup-parameter[Microsoft's component cleanup] page to clear previous versions of Windows components.
. *More RAM will be used* to increase the general performance of Windows, such as paging more operations to prevent creating more hard page faults, which are perceived as stalling/temporary freezing.
...
- W11Boost does no apparent changes, which is intentional. To see that changes have applied, search then open `rsop.msc` from the Start Menu.
