...
WARNING: Only Windows 10 Enterprise LTSC 2021 and Windows 11 Enterprise LTSC 2024 (for both, also includes IoT versions) are supported, as they don't revert changes on updates it seems. +
You can download this special Windows 11 version from: https://forums.mydigitallife.net/posts/1837518 +
Use https://rufus.ie to write that ISO to a USB, so you can install that Windows.
...
. Doesn't ruin your security unlike AtlasOS. It can be argued however that W11Boost making updates not be forcefully installed is less secure.
