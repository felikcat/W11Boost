...
-> What will be done to solve this: W11Boost is being rewritten in https://ziglang.org/[Zig], but if Zig doesn't work out then the https://github.com/felikcat/W11Boost/tree/rust[Rust version] gets priority.
...
- Power Throttling, as it causes severe performance reduction for VMWare Workstation 17 and other apps I'm unaware of.
...
. Boot from the USB you flashed that Windows ISO onto. When prompted to "Activate Windows", click "I don't have a product key". Afterwards, select "Windows 11 Education".

. image:WinSetup1.png[]

. image:WinSetup2.png[]

. After Windows 11 Education is installed, continue to installing W11Boost.
