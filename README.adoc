...
** Fully open-source except for 'PolicyFileEditor'; a closed-source DLL that's easily reversed, has been around for a long time, and no antivirus flags it as malicious.
...
== The catches
. Windows Home editions are not supported; see link:https://github.com/Fleex255/PolicyPlus#special-considerations-for-use-on-home-editions[this excerpt] from PolicyPlus on why.
