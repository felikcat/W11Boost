...
int disable_sleep()
{
    ...
    set_dword(hKey, LR"(SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\FlyoutMenuSettings)", L"ShowHibernateOption",
              0);

    // Disable the Sleep entry in the Power Menu
    set_dword(hKey, LR"(SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\FlyoutMenuSettings)", L"ShowSleepOption", 0);
