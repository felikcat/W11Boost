...
auto restorepoint_prep() -> void
{
    ...
    if (long_result == ERROR_SUCCESS)
    {
        ...
        RegSetValueExW(hSubKey, L"SystemRestorePointCreationFrequency", 0, REG_DWORD,
                       std::bit_cast<const BYTE*>(&value), sizeof(value));
    }
...
auto create_restore_point() -> int
{
    ...
    if (!fRet)
    { // Either SR is off, or restore point creation failed outright
        failure = TRUE;
        goto exit;
    }
