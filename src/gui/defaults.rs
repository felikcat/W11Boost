...
pub fn run() -> Result<(), Box<dyn Error>> {
        ...
        set_dword(&hklm, r"SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System", "verbosestatus", 1)?;

        // Ask to not allow execution of experiments by Microsoft.
        set_dword(&hklm, r"SOFTWARE\Microsoft\PolicyManager\current\device\System", "AllowExperimentation", 0)?;
        ...
        set_dword(&hklm, r"SOFTWARE\Policies\Microsoft\Windows\Appx", "AllowStorageSenseGlobal", 0)?;
        ...
        set_dword(&hklm, r"SYSTEM\CurrentControlSet\Policies", "NtfsForceNonPagedPoolAllocation", 1)?;

        Command::new("fsutil.exe").args(["behavior", "set", "memoryusage", "2"]).output()?;

        // Disable automatic repair to instead ask for a repair.
        // Does not disable Windows' Recovery environment thankfully.
        Command::new("bcdedit.exe").args(["/set", "{default}", "recoveryenabled", "no"]).output()?;
        ...
        set_dword(&hklm, r"SOFTWARE\Policies\Microsoft\Windows\System", "HiberbootEnabled", 0)?;
        ...
        SetFileAttributes(file_path, FILE_ATTRIBUTE::ARCHIVE | FILE_ATTRIBUTE::READONLY).expect("SetFileAttributes 2 failed");
        ...
        set_dword(&hklm, r"SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\DataCollection", "AllowTelemetry", 0)?;
        set_dword(
                &hklm,
        ...
        set_dword(&hklm, r"SOFTWARE\Policies\Microsoft\Windows\AdvertisingInfo", "DisabledByGroupPolicy", 1)?;
        set_dword(&hklm, r"SOFTWARE\Policies\Microsoft\Windows\DataCollection", "AllowDeviceNameInTelemetry", 0)?;
        set_dword(&hklm, r"SOFTWARE\Policies\Microsoft\Windows\DataCollection", "AllowTelemetry", 0)?;
        ...
        set_dword(&hklm, r"SOFTWARE\Policies\Microsoft\Windows\DataCollection", "DisableOneSettingsDownloads", 1)?;
        ...
        set_dword(&hklm, r"SOFTWARE\Policies\Microsoft\Windows\Windows Error Reporting", "Disabled", 1)?;
        ...
        set_string(&hklm, r"SOFTWARE\Policies\Microsoft\PCHealth\ErrorReporting\ExclusionList", "*", "*")?;

        set_dword(&hklm, r"SYSTEM\CurrentControlSet\Services\WerSvc", "Start", 4)?;

        remove_subkey(&hklm, r"SOFTWARE\Policies\Microsoft\PCHealth\ErrorReporting\InclusionList")?;
        ...
        set_dword(&hklm, r"SOFTWARE\Policies\Microsoft\Windows\TabletPC", "PreventHandwritingDataSharing", 1)?;
        ...
        set_dword(&hklm, r"SOFTWARE\Microsoft\OneDrive", "PreventNetworkTrafficPreUserSignIn", 1)?;

        // Don't ask to change the current privacy settings after applying a major
        // Windows update
        set_dword(&hklm, r"SOFTWARE\Policies\Microsoft\Windows\OOBE", "DisablePrivacyExperience", 1)?;

        set_dword(&hkcu, r"Software\Microsoft\Windows\CurrentVersion\AdvertisingInfo", "Enabled", 0)?;
        set_dword(&hkcu, r"Software\Microsoft\Speech_OneCore\Settings\OnlineSpeechPrivacy", "HasAccepted", 0)?;
        set_dword(&hkcu, r"Software\Microsoft\InputPersonalization", "RestrictImplicitInkCollection", 1)?;
        set_dword(&hkcu, r"Software\Microsoft\InputPersonalization", "RestrictImplicitTextCollection", 1)?;
        ...
        set_dword(&hklm, r"SOFTWARE\Policies\Microsoft\Windows\WTDS\Components", "ServiceEnabled", 0)?;

        // Disable the language list fingerprinting method; useful for bypassing geolocation restrictions.
        set_dword(&hkcu, r"Control Panel\International\User Profile", "HttpAcceptLanguageOptOut", 1)?;

        // Enable multiple processes for explorer.exe for increased stability and performance.
        set_dword(&hkcu, r"Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced", "SeparateProcess", 1)?;

        // Hidden file extensions are abused to hide the real file format, example:
        // an executable (.exe, .scr, .com) pretending to be a PDF.
        set_dword(&hkcu, r"Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced", "HideFileExt", 0)?;

        // Allow usage of some .appx/.appxbundle apps without a Microsoft account.
        set_dword(&hklm, r"SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System", "MSAOptional", 1)?;
